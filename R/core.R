...
collect <- function(type = "collect_id") {
    ...
    if (type == "collect_id") {
        ...
        mc <- mongolite::mongo('match', 'dota')
        match_id_collected <- mc$find(, fields = '{"_id": 1}')
        if (nrow(match_id_collected) > 0) {
            match_id_collected <- match_id_collected$`_id`
        }
        mc$disconnect()
        ...
        match_id <- match_id[!(match_id %in% match_id_collected)]
    ...
    } else if (type == "clean_match_id") {
        delete_match <- match_id[!(match_id %in%  match_id_not_collected)]
